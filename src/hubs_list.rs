use std::{ops::DerefMut, sync::{Arc, RwLock, atomic::{AtomicBool, AtomicU8, Ordering}}};

use eframe::egui::{Button, Context, Image, Label, Layout, RichText, ScrollArea, Spinner, Ui, Vec2};
...
impl Default for HubsList {
    fn default() -> Self {
        let tokio_rt = tokio::runtime::Builder::new_multi_thread().enable_time().enable_io().build().unwrap();
...
impl HubsList {
    pub fn ui(&mut self, ui: &mut Ui, _ctx: &Context) {
        let paging_height = 40.0;
        let paging_padding = 10.0;
        ui.with_layout(Layout::top_down(eframe::egui::Align::Center), |ui| {
            ...
            if self.is_loading.load(Ordering::Relaxed) {
                ui.add_sized(ui.available_size() - Vec2::new(0., paging_height + (paging_padding * 2.)), Spinner::new().size(32.0));
            } else {
                ...
                scroll_area.show(ui, |ui| {
                        for hub in self.hubs.read().unwrap().iter() {
                            ui.add_space(10.0);
                            ui.horizontal(|ui| {
                                ui.add(Image::new("https:".to_string() + hub.image_url.as_str())
                                    .fit_to_exact_size((80.0, 80.0).into())
                                );
                                ui.vertical(|ui| {
                                    ui.label(RichText::new(hub.title.as_str())
                                        .size(18.0)
                                        .strong()
                                    );
                                    ui.label(hub.description_html.as_str());
                                })
                            });
            ...
            Flex::horizontal().w_full().align_content(egui_flex::FlexAlignContent::SpaceBetween).show(ui, |flex_ui| {
                ...
                flex_ui.add(item().grow(1.), Label::new(RichText::new(format!("{}/{}", self.current_page, max_page)).size(32.)));
